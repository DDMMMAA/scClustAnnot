...
# Perform clustering sweep from resolution 0.1 to 0.8 with margin = 0.1
...
# at least 25% of cell with in and out each cluster is expressing
# Unregulated at least 0.25 fold in cluster
...
`Note`: Since automated annotation are mostly done by map to previously annotated data, so the quality and relativity of previously
annotated data is a great concern, choice it wisely.
...
All avaliable reference can be found [here](https://www.bioconductor.org/packages/release/data/experiment/vignettes/celldex/inst/doc/userguide.html)
...
All avaliable reference can be found [here](https://www.bioconductor.org/packages/release/data/experiment/vignettes/clustifyrdatahub/inst/doc/clustifyrdatahub.html)
...
org_db = `"org.Hs.eg.db"` (Homo sapiens), or `"org.Mm.eg.db"` (Mus musculus)

ont = `"BP"` (Biological Process), `"MF"` (Molecular Function), `"CC"` (Cellular Component)

The `ClusterGo` function maps marker genes to biological pathways. In the resulting dotplot or barplot, look for terms on the y-axis with a high GeneRatio and a low p.adjust (typically red), which indicate highly significant biological activities.

To manually annotate a cluster, combine these functional insights with the marker genes from Step 2. For example, if Cluster 0 is enriched for GO terms like "T cell activation" or "lymphocyte differentiation" and expresses known markers like CD3D, you can confidently annotate this cluster as T Cells.

```{r cluster-go, eval=requireNamespace("org.Hs.eg.db", quietly=TRUE), fig.height=5}
# Run GO analysis
# Note: Requires org.Hs.eg.db to be installed
go_results <- ClusterGo(deg_results, org_db = "org.Hs.eg.db", ont = "BP")

# Visualize enrichment for Cluster 0 using dotplot
enrichplot::dotplot(go_results$cluster_0, showCategory = 10) + 
  ggtitle("GO Enrichment: Cluster 0")
# or barplot
barplot(go_results$cluster_0, showCategory = 10) + 
  ggtitle("GO Enrichment: Cluster 0")
```
